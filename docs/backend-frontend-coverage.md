...
| Subscriptions & billing | `GET /billing/prices`, `GET /billing/entitlement`, `GET /billing/eligibility`; `POST /billing/checkout`, `POST /billing/portal` | Authenticated | Subscription plan selection, Stripe Checkout sessions, Customer Portal management, feature entitlement, and trial eligibility |
...
| Blockchain, KYC, Stripe, and inbox webhook routes | Backend-only | Inbound provider callbacks must never be invoked from the browser. |
...
- The API sends `Cache-Control: no-store` on every `/api/*` response and exposes no ETag, so the browser cache is unavailable and the React Query client cache is the primary client-side cache. Its `staleTime` mirrors the server's in-memory LRU TTLs (60s for analytics and payment totals, 5 min for skill trends and platform metrics).
- The backend actively triggers targeted cache invalidation hooks upon entity and payment mutations (e.g. `invalidatePaymentSummaryCache`, `deleteMatching`), ensuring that client refetches following state mutations return current data.
