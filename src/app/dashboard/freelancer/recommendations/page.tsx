...
export default function FreelancerRecommendationsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <div className="flex h-64 items-center justify-center" aria-label="Loading recommendations"><Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /></div> : recommendations.length === 0 ? <Card><CardContent className="flex flex-col items-center gap-3 py-16 text-center"><FolderSearch className="h-10 w-10 text-muted-foreground" /><div><p className="font-medium">No recommendations yet</p><p className="mt-1 text-sm text-muted-foreground">Add more skills to your profile or browse all open projects.</p></div><Button asChild variant="outline"><Link href="/dashboard/freelancer/projects">Browse projects</Link></Button></CardContent></Card> : <div className="grid gap-4 lg:grid-cols-2">{recommendations.map((item) => <Card key={item.projectId}><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle>{item.project.title}</CardTitle><p className="mt-1 text-sm text-muted-foreground">${item.project.budget.toLocaleString()} · due {new Date(item.project.deadline).toLocaleDateString()}</p></div><Badge className="bg-success-subtle text-success">{Math.round(item.matchScore)}% match</Badge></div></CardHeader><CardContent className="space-y-4"><p className="line-clamp-2 text-sm text-muted-foreground">{item.project.description}</p><div><p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">Matched skills</p><div className="flex flex-wrap gap-2">{item.matchedSkills.map((skill) => <Badge key={skill} variant="secondary">{skill}</Badge>)}</div></div>{item.missingSkills.length > 0 && <p className="text-xs text-muted-foreground">Potential gaps: {item.missingSkills.join(', ')}</p>}<p className="rounded-lg bg-secondary/40 p-3 text-sm text-muted-foreground">{item.reasoning}</p><Button asChild><Link href={`/projects/${item.project.id}`}>View project<ArrowUpRight className="ml-2 h-4 w-4" /></Link></Button></CardContent></Card>)}</div>}
    </div>
  );
}
