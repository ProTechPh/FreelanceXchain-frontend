...
export function ReputationOverview() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="relative overflow-hidden border-border bg-card"><div className="absolute inset-0 gradient-primary opacity-5" /><CardContent className="relative grid gap-8 p-6 md:grid-cols-[auto_1fr] md:items-center"><div className="text-center"><div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full gradient-primary"><span className="text-3xl font-bold text-primary-foreground">{overall.toFixed(1)}</span></div><div className="mt-2 flex items-center justify-center gap-1">{[1, 2, 3, 4, 5].map((star) => <Star key={star} className={`h-4 w-4 ${star <= Math.round(overall) ? 'fill-warning text-warning' : 'text-neutral'}`} />)}</div><p className="mt-1 text-sm text-muted-foreground">{totalRatings} review{totalRatings === 1 ? '' : 's'}</p></div><div className="space-y-2">{starKeys.map(([stars, key]) => { const count = breakdown?.[key] ?? 0; return <div key={stars} className="flex items-center gap-3"><span className="w-4 text-sm">{stars}</span><Star className="h-4 w-4 fill-warning text-warning" /><div className="h-2 flex-1 overflow-hidden rounded-full bg-background"><div className="h-full rounded-full bg-warning" style={{ width: totalRatings > 0 ? `${(count / totalRatings) * 100}%` : '0%' }} /></div><span className="w-8 text-sm text-muted-foreground">{count}</span></div>; })}</div></CardContent></Card>
