...
<Textarea id="category-description" value={categoryDescription} onChange={(event) => setCategoryDescription(event.target.value)} />
</Field><Button type="submit" disabled={action === 'category'}><Plus className="mr-2 size-4" />Create category</Button></form></CardContent></Card>
...
<select id="skill-category" className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm" value={skillCategoryId} onChange={(event) => setSkillCategoryId(event.target.value)}>{taxonomy.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select>
</Field><Field label="Name" htmlFor="admin-skill-name">
<Input id="admin-skill-name" value={skillName} onChange={(event) => setSkillName(event.target.value)} />
</Field><Field label="Description" htmlFor="admin-skill-description">
<Textarea id="admin-skill-description" value={skillDescription} onChange={(event) => setSkillDescription(event.target.value)} />
</Field><Button type="submit" disabled={action === 'skill'}><Plus className="mr-2 size-4" />Create skill</Button></form></CardContent></Card>
      </div>
